...
Example uses:
  ...
  format_code.py src.c
    Formats the file in-place.

  format_code.py -o src.c
    Performs out-of-place formatting (creates new file src_formatted.c).
...
parser.add_argument(
  '-o', '--outplace', action='store_true', help='Create new files')
parser.add_argument(
  '-a', '--all', action='store_true', help='Format the entire codebase')
parser.add_argument(
  '-s', '--silent', action='store_true', help='Suppress verbose output')
parser.add_argument(
  '-j', '--jobs', action='store', help='Amount of parallel jobs', default='1')
parser.add_argument(
  '-n', '--dry-run', action='store_true',
  help='Just check if files need formatting')
parser.add_argument(
  '-Werror', '--Werror', action='store_true', help='Treat warnings as errors.')
parser.add_argument(
  'args', nargs='*', action='store', help='Files to process')
